...
var dashCalYear, dashCalMonth;
var fcMode = 'manage';
var fcFolderFilter = 'alle';
var fcModuleFilter = '';
var docFolderFilter = 'alle';
let studyIndex = 0;
let studyOrder = [];
let flipped = false;
let studyResults = {
...
function getModuleColor(moduleId, alpha = 1){
  const pastelColors = [
    { hue: 55, sat: 15, light: 93 },
    { hue: 35, sat: 18, light: 92 },
    { hue: 30, sat: 15, light: 92 },
    { hue: 355, sat: 12, light: 92 },
    { hue: 70, sat: 15, light: 91 },
    { hue: 120, sat: 18, light: 91 },
    { hue: 170, sat: 15, light: 91 },
    { hue: 200, sat: 15, light: 91 },
    { hue: 210, sat: 15, light: 90 },
    { hue: 270, sat: 15, light: 91 },
    { hue: 320, sat: 15, light: 91 },
    { hue: 350, sat: 12, light: 92 },
    { hue: 30, sat: 12, light: 92 },
    { hue: 45, sat: 10, light: 93 },
    { hue: 0, sat: 8, light: 94 },
    { hue: 200, sat: 10, light: 92 }
  ];
  let hash = 0;
...
(function installFinalDocCategoryFix(){
  ...
  window.renderNoteModuleFilter = function(){
    const wrap = document.getElementById('note-module-filter');
    if(!wrap) return;
    const options = [{id:'',name:'Alle Module'},...(Array.isArray(modules)?modules:[])];
    wrap.innerHTML = `<select onchange="setNoteModuleFilter(this.value)" style="padding:8px 12px;border:1px solid var(--line);border-radius:8px;background:var(--bg-soft);color:var(--ink);font-weight:600;cursor:pointer;">${options.map(mod=>`<option value="${escapeHtml(mod.id)}" ${noteModuleFilter===mod.id?'selected':''}>${escapeHtml(mod.name||mod.title||'Alle Module')}</option>`).join('')}</select>`;
  };

  window.renderFlashcardModuleFilter = function(){
    const wrap = document.getElementById('fc-module-filter');
    if(!wrap) return;
    const options = [{id:'',name:'Alle Module'},...(Array.isArray(modules)?modules:[])];
